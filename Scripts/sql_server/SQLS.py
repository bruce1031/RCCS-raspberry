...
if __name__ == '__main__':
    d = 0
    D = 0
    col_Table = ['id', 'takeoff', 'land', 'massage', 'status', 'dronemode',
                 'up_down', 'lat_lon', 'droneturn', 'forward_back', 'allmove',
                 'cam', 'dronebatt', 'GPSInfo', 'allmove_FW', 'allmove_LR', 'allmove_yaw']
    ...
    while True:
        ...
        if event == 'ud':
            print('欄位名稱: 1-id , 2-takeoff , 3-land , 4-massage , 5-status , 6-dronemode , 7-up_down , 8-lat_lon , 9-droneturn , 10-forward_back , 11-allmove , 12-cam , 13-dronebatt , 14-GPSInfo',
                  '15-allmove_FW', '16-allmove_LR', '17-allmove_yaw')
            a = input('輸入欄位名稱or編號:')
            try:
                a = int(a)
